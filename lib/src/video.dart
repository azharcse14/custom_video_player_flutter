...
import 'package:custom_video_player/src/utils/utils.dart';
import 'package:custom_video_player/src/widget/widget_bottombar.dart';
...
// M3U8 Data Setup
  void getM3U8(String video) {
    if (customPlayer.length > 0) {
      print("${customPlayer.length} : data start clean");
      m3u8clean();
    }
  ...
  Future<M3U8s> m3u8video(String video) async {
    customPlayer.add(M3U8pass(dataQuality: "Auto", dataURL: video));
    ...
    print(
        "--- HLS Data ----\n$m3u8Content \ntotal length: ${customPlayer.length} \nfinish");
    ...
    matches.forEach(
      (RegExpMatch regExpMatch) async {
        ...
        try {
          ...
          final File file = File('${directory.path}/customPlayer$quality.m3u8');
          await file.writeAsString(
              """#EXTM3U\n#EXT-X-INDEPENDENT-SEGMENTS\n$audio#EXT-X-STREAM-INF:CLOSED-CAPTIONS=NONE,BANDWIDTH=1469712,RESOLUTION=$quality,FRAME-RATE=30.000\n$url""");
        } catch (e) {
          print("Couldn't write file");
        }
        customPlayer.add(M3U8pass(dataQuality: quality, dataURL: url));
      },
    );
    M3U8s m3u8s = M3U8s(m3u8s: customPlayer);
    print(
        "--- m3u8 file write ---\n${customPlayer.map((e) => e.dataQuality == e.dataURL).toList()}\nlength : ${customPlayer.length}\nSuccess");
    return m3u8s;
  }
...
// video Listener
  ...
  void onSelectQuality(M3U8pass data) async {
    ...
    if (data.dataQuality == "Auto") {
      ...
      try {
        ...
        final Directory directory = await getApplicationDocumentsDirectory();
        final File file =
            File('${directory.path}/customPlayer${data.dataQuality}.m3u8');
  ...
  void m3u8clean() async {
    print(customPlayer.length);
    for (int i = 2; i < customPlayer.length; i++) {
      try {
        final Directory directory = await getApplicationDocumentsDirectory();
        final File file =
            File('${directory.path}/${customPlayer[i].dataQuality}.m3u8');
        file.delete();
